...
def _itunes_explicit(*flags) -> Optional[bool]:
    """Map iTunes explicitness fields to None/True/False.

    iTunes uses ``explicit`` / ``cleaned`` / ``notExplicit`` values. Prefers the
    first non-empty field (tracks use ``trackExplicitness`` first, then
    ``collectionExplicitness``). Returns None when unknown so callers degrade to
    "unknown" rather than guessing (FR-005).
    """
    for flag in flags:
        if not flag:
            continue
        ...
        if val == "explicit":
            return True
        if val in ("cleaned", "notexplicit", "not explicit"):
            return False
    return None
...
class SpotifyClient:
    ...
    def search_music(self, query: str, limit: int = 8) -> list[dict]:
        """Search songs and albums together, returning categorized results.

        Each result dict:
            {type, title, artist, album, artwork_url, track_url}
        where ``type`` is "track" or "album". Uses the authenticated Spotipy
        client when available; otherwise falls back to the public iTunes Search
        API. Empty list if nothing found.
        """
        results: list[dict] = []

        if self.sp:
            try:
                raw = self.sp.search(q=query, type="track,album", limit=limit, market=self._market)

                for it in raw.get("tracks", {}).get("items", []) or []:
                    artists = [a.get("name", "") for a in it.get("artists", []) if a.get("name")]
                    ...
                    results.append({
                        ...
                        "source": "spotify",
                        # True = explicit, False = clean/edited, None/absent = unknown.
                ...
                for item in raw.get("albums", {}).get("items", []) or []:
                    ...
                    label = {"single": "Single", "album": "Album"}.get(album_type, album_type.capitalize() or "Album")
                    results.append({
                        "type": "album",
                        ...
                        "subtitle": label,
                        "artwork_url": artwork,
                        "track_url": f"https://open.spotify.com/album/{album_id}" if album_id else "",
                        "source": "spotify",
                    })
                ...
                logger.debug(f"[Spotify] Multi music search failed for '{query}': {e}")

        # Anonymous Spotify fallback — tries the web-player token so results still
        # carry the explicit/clean flag, before giving up on the public iTunes API.
        if not results:
            try:
                results = self._search_music_anonymous(query, limit)
            except Exception as e:  # noqa: BLE001
                logger.debug(f"[Spotify] Anonymous multi music search failed for '{query}': {e}")
            if results:
                return results

        # Fallback: public iTunes Search API (no auth) — search songs and albums.
        # Uses a larger limit and best-effort refinement (dedup, drop covers,
        # infer explicit from title) because iTunes search returns a lot of noise.
        try:
            import requests
            from antra.utils.search_refine import refine_itunes_results

            resp = requests.get(
                "https://itunes.apple.com/search",
                params={"term": query, "entity": "song", "limit": max(limit * 3, 25), "media": "music"},
                timeout=10,
            )
            resp.raise_for_status()
            ...
            for item in resp.json().get("results", []):
                ...
                song_results.append({
                    "type": "track",
                    "title": item.get("trackName") or item.get("collectionName") or "Unknown Track",
                    "artist": item.get("artistName") or "Unknown Artist",
                    "album": item.get("collectionName") or "Unknown Album",
                    "subtitle": "",
                    "artwork_url": artwork,
                    "track_url": item.get("trackViewUrl") or item.get("collectionViewUrl") or "",
                    "source": "itunes",
                    # iTunes exposes explicit/cleaned/notExplicit via the ASIN fields.
                    "is_explicit": _itunes_explicit(
                        item.get("trackExplicitness"),
                        item.get("collectionExplicitness"),
                    ),
                })
            results.extend(refine_itunes_results(song_results))

            resp2 = requests.get(
                "https://itunes.apple.com/search",
                params={"term": query, "entity": "album", "limit": max(limit * 2, 15), "media": "music"},
                timeout=10,
            )
            ...
            for item in resp2.json().get("results", []):
                ...
                album_results.append({
                    ...
                    "subtitle": "",
                    "artwork_url": artwork,
                    "track_url": item.get("collectionViewUrl") or "",
                    "source": "itunes",
                    "is_explicit": _itunes_explicit(item.get("collectionExplicitness"), None),
                })
            results.extend(refine_itunes_results(album_results))

            return results
        except Exception as e:  # noqa: BLE001
            logger.debug(f"[Spotify] iTunes multi music search failed for '{query}': {e}")
            return []

    def _search_music_anonymous(self, query: str, limit: int) -> list[dict]:
        """Search songs and albums via Spotify's web API using an anonymous token.

        Mirrors the authenticated ``self.sp`` result shape but never requires creds,
        and — crucially — returns the ``explicit`` flag per track so results are not
        all marked "unknown". Returns an empty list if no anonymous token is
        available or the request fails.
        """
        token = self._get_anonymous_token() or self._get_anonymous_access_token()
        if not token:
            return []
        ...
        raw = requests.get(
            "https://api.spotify.com/v1/search",
            headers={"Authorization": f"Bearer {token}"},
            params={"q": query, "type": "track,album", "limit": limit, "market": self._market or "US"},
            timeout=10,
        )
        raw.raise_for_status()
        data = raw.json()

        for it in data.get("tracks", {}).get("items", []) or []:
            artists = [a.get("name", "") for a in it.get("artists", []) if a.get("name")]
            images = it.get("album", {}).get("images", []) or []
            artwork = images[0]["url"] if images else ""
            track_id = it.get("id")
